...
export function EditableHomeHero({ primaryTask, primaryRoute, posts }: HomeSectionProps) {
  ...
  return (
    <section className="signal-on-dark relative overflow-hidden bg-[var(--slot4-dark-bg)] text-white">
      <div className="pointer-events-none absolute -right-32 -top-32 h-[34rem] w-[34rem] rounded-full bg-[var(--slot4-accent)] opacity-30 blur-[120px]" />
      <div className="pointer-events-none absolute -bottom-40 left-1/4 h-[30rem] w-[30rem] rounded-full bg-[var(--slot4-accent-2)] opacity-20 blur-[120px]" />
      <div className={`relative ${dc.shell.section} py-16 sm:py-20 lg:py-28`}>
        <div className="grid items-center gap-12 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="signal-rise">
            <span className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-[11px] font-extrabold uppercase tracking-[0.18em] text-white/85">
              <Sparkles className="h-3.5 w-3.5 text-[var(--slot4-accent-2)]" /> {hero.badge}
            </span>
            <h1 className="mt-6 text-4xl font-extrabold leading-[1.04] tracking-[-0.035em] sm:text-5xl lg:text-6xl">
              {hero.title[0]} <span className="bg-[linear-gradient(120deg,#5eead4,#fbbf24)] bg-clip-text text-transparent">{hero.title.slice(1).join(' ')}</span>
            </h1>
            <p className="mt-6 max-w-xl text-base leading-8 text-white/70 sm:text-lg">{hero.description}</p>

            <form action="/search" className="mt-8 flex max-w-xl items-center gap-2 rounded-full border border-white/15 bg-white/10 p-1.5 backdrop-blur">
              <Search className="ml-3 h-5 w-5 text-white/55" />
              <input name="q" placeholder={hero.searchPlaceholder} className="min-w-0 flex-1 bg-transparent px-2 py-2.5 text-sm text-white outline-none placeholder:text-white/45" />
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <Link href={hero.primaryCta.href} className="inline-flex items-center gap-2 rounded-full bg-[var(--slot4-accent-2)] px-7 py-3.5 text-sm font-bold text-[var(--slot4-dark-bg)] shadow-[0_14px_34px_rgba(245,158,11,0.3)] transition hover:-translate-y-0.5">
                {hero.primaryCta.label} <ArrowRight className="h-4 w-4" />
              </Link>
              <Link href={hero.secondaryCta.href} className="inline-flex items-center gap-2 rounded-full border border-white/25 px-7 py-3.5 text-sm font-bold text-white transition hover:bg-white/10">
                {hero.secondaryCta.label}
              </Link>
            </div>
          </div>

          <aside className="signal-rise rounded-3xl border border-white/12 bg-white/[0.06] p-6 backdrop-blur-xl sm:p-7" style={{ animationDelay: '120ms' }}>
            <div className="flex items-center justify-between">
              <span className="inline-flex items-center gap-2 text-sm font-bold text-white/85"><span className="relative flex h-2.5 w-2.5"><span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-[var(--slot4-accent-2)] opacity-75" /><span className="relative inline-flex h-2.5 w-2.5 rounded-full bg-[var(--slot4-accent-2)]" /></span> {hero.focusLabel}</span>
              <Link href={primaryRoute} className="text-xs font-bold text-white/55 transition hover:text-white">View all</Link>
            </div>
            <div className="mt-4 grid gap-1">
              {list.length ? list.map((post, index) => (
                <Link key={post.id || post.slug} href={postHref(primaryTask, post, primaryRoute)} className="group grid grid-cols-[34px_1fr] items-start gap-3 rounded-2xl px-2 py-3 transition hover:bg-white/10">
                  <span className="text-lg font-extrabold text-[var(--slot4-accent-2)]">{String(index + 1).padStart(2, '0')}</span>
                  <span className="min-w-0">
                    <span className="block text-[11px] font-extrabold uppercase tracking-[0.12em] text-white/45">{(post.content as Record<string, unknown>)?.category as string || 'Newswire'}</span>
                    <span className="mt-1 block line-clamp-2 text-sm font-bold leading-snug text-white/90 transition group-hover:text-white">{post.title}</span>
                  </span>
                </Link>
              )) : (
                <p className="rounded-2xl bg-white/5 px-4 py-8 text-center text-sm text-white/55">Fresh releases will appear here as they cross the wire.</p>
              )}
            </div>
          </aside>
        </div>
        ...
        <div className="mt-14 grid grid-cols-2 gap-4 border-t border-white/10 pt-10 sm:grid-cols-4">
          {hero.stats.map((stat, index) => (
            <div key={stat.label} data-reveal style={delay(index)} className="text-center sm:text-left">
        ...
        <div className="signal-on-dark relative overflow-hidden rounded-[2rem] bg-[var(--slot4-dark-bg)] px-6 py-14 text-white sm:px-12 lg:px-16 lg:py-20" data-reveal>
          ...
          <div className="relative grid items-center gap-8 lg:grid-cols-[1.3fr_0.7fr]">
            ...
            <div className="flex flex-col gap-3 lg:items-end">
              <Link href={cta.primaryCta.href} className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-[var(--slot4-accent-2)] px-7 py-4 text-sm font-bold text-[var(--slot4-dark-bg)] shadow-[0_14px_34px_rgba(245,158,11,0.3)] transition hover:-translate-y-0.5 lg:w-auto">{cta.primaryCta.label} <ArrowRight className="h-4 w-4" /></Link>
