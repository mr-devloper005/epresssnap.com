...
const inputClass = 'h-[3.25rem] rounded-2xl border border-[var(--editable-border)] bg-white px-4 text-base font-semibold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)] focus:ring-4 focus:ring-[var(--slot4-ring)]'
const buttonClass = 'inline-flex h-[3.25rem] items-center justify-center rounded-full bg-[var(--slot4-accent)] px-6 text-sm font-bold tracking-[0.02em] text-white shadow-[0_12px_30px_rgba(13,148,136,0.28)] transition hover:-translate-y-0.5 hover:bg-[var(--slot4-accent-strong)] disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-2xl border px-4 py-3 text-sm font-semibold ${status === 'success' ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-600'}`}>{message}</p> : null}
